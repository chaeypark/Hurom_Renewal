...
public class PassController0 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//pass 뷰 = 수정/삭제를 눌렀을 때 비밀번호확인하는 페이지 
		req.getRequestDispatcher("/pages/CSMain_pass.jsp").forward(req, resp);
		//MVCBoard에 비밀번호가 일치하는지 아닌지 알아보기 위한 Pass.jsp 뷰생성
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(confirmed ) {
			...
			if(mode.equals("edit")) { //수정16. 수정모드
				...
				//수정19. 수정하기 페이지로 이동
				resp.sendRedirect("../mvc2/edit0.do?idx=" + idx);
				
			}else if(mode.equals("delete")) { //삭제16. 삭제모드
				
				//삭제17. 삭제를 선택하면 게시글에 첨부된 파일도 함께 삭제처리해야함.
				dao = new CSMainBoardDAO();
				
				//삭제20. 기존 정보를 보관
				CSMainBoardDTO dto = dao.selectView(idx);
				...
				//삭제34 알림창표시  참고로 mvc/list.do customer service 페이지임
				JSFunction.alertLocation(resp, "삭제되었습니다.", "../mvc2/list.do");
			}
