...
//뷰에서 할 일 > 결과를 화면에 보여줌 == 다오 생성하고 조회수 +1 보여줌
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		CSMainBoardDAO dao  = new CSMainBoardDAO(); //상세보기4. dao 라는 객체생성
		String idx = req.getParameter("idx"); //상세보기5. 게시글의 일련번호를 매개변수로 받아서	
		dao.updateVisitCount(idx);				//상세보기9. 해당하는 게시글에 대한 조회수 +1 
		CSMainBoardDTO dto = dao.selectView(idx); //상세보기16. 게시글의 내용을 가져옴
		dao.close();
		...
		//상세보기19. 뷰로 포워드 시킴.
		req.getRequestDispatcher("/pages/CSMain_view.jsp").forward(req, resp);
