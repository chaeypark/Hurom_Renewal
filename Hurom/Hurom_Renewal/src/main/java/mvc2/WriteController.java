...
public class WriteController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		req.getRequestDispatcher("/pages/Customer_service_q&a.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(mr == null) {
			...
			JSFunction.alertLocation(resp,"첨부 파일이 용량을 초과했습니다.", "../mvc2/write.do");
			return;
		}
		...
		if(fileName != null) {
			...
			if(result == 1 ) { //성공이면 
				resp.sendRedirect("../mvc2/listQNA.do"); //성공하면 목록으로 
			}else { //실패면 
				//글쓰기 실패시 다시 글쓰기 창으로 보내줌
				resp.sendRedirect("../mvc2/write.do"); //글쓰기 창으로 
			}
			//첨부파일이 없을 때
		}else {
			//글쓰기20. DAO를 통해 DB에 작성된 내용을 저장
