...
public class JuicerBoardDAO extends DBConnPool {

	public JuicerBoardDAO() {
		super();
	}
	...
	public int selectCount(Map<String, Object> map) {
		...
		// 목록10
		// 쿼리문 생성
		String query = "SELECT COUNT(*) FROM PRODUCT WHERE P_CAT = 'juice_maker'";

		// 목록11
		// 검색 조건이 있으면 해당 쿼리문을 붙여서 실행
		if (map.get("searchWord") != null) {
			query += " AND WHERE " + map.get("searchField") + " LIKE '%" + map.get("searchWord") + "%'";
		}
...
//				+ "		SELECT * FROM PRODUCT WHERE P_CAT LIKE 'juice%' 	";
//			
//			 if (map.get("searchWord") != null) { 
...
//			 
//				/*
//				 * query += "ORDER BY product_num " + "	) tb " + " )" +
//				 * " WHERE rNum BETWEEN ? AND ? ";
//				 */
//			 	
//				 query +=   " ORDER BY product_num "
//							+ "	) tb "
//							+ " )"
//							+ " WHERE rNum BETWEEN ? AND ? ";
		...
		query += " Where p_cat = 'juice_maker' ) tb ) WHERE rNum BETWEEN ? AND ?";
		...
		try {
			...
			while (rs.next()) {
				...
				dto.setUrl_add(rs.getString("url_add"));
